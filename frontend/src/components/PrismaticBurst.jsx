...
float edgeFade(vec2 frag, vec2 res, vec2 offset){
    ...
    float r = length(toC) / (uFadeRadius * 0.5 * min(res.x, res.y));
...
const PrismaticBurst = ({
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      try { meshRef.current?.remove?.(); } catch (e) { void e; }
      try { triRef.current?.remove?.(); } catch (e) { void e; }
      try { programRef.current?.remove?.(); } catch (e) { void e; }
      try {
  ...
  useEffect(() => {
    ...
    program.uniforms.uFadeRadius.value = Math.max(0.1, fadeRadius ?? 1.0);
  ...
  }, [intensity, speed, animationType, colors, distort, offset, rayCount, fadeRadius]);
