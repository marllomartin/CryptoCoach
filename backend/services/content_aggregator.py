# Premium Content Aggregator
# Combines all lesson content from separate files

from typing import Dict, List, Optional

# Import all content modules
from services.premium_content_service import PREMIUM_LESSONS, PREMIUM_COURSES
...
ALL_PREMIUM_LESSONS = {
    **PREMIUM_LESSONS,      # Lessons 1-2 from premium_content_service
...
def get_localized_lesson(lesson_id: str, language: str = "en") -> Optional[Dict]:
    ...
    lesson = ALL_PREMIUM_LESSONS[lesson_id].copy()
...
def get_all_lessons_for_course(course_id: str, language: str = "en") -> List[Dict]:
    ...
    lessons = []
    for lesson_id, lesson in ALL_PREMIUM_LESSONS.items():
...
def get_premium_content_stats() -> Dict:
    """Get statistics about premium content"""
    return {
        "total_courses": len(PREMIUM_COURSES),
        ...
        "courses": {
            "course-foundations": len([l for l in ALL_PREMIUM_LESSONS.values() if l.get("course_id") == "course-foundations"]),
            "course-investor": len([l for l in ALL_PREMIUM_LESSONS.values() if l.get("course_id") == "course-investor"]),
            "course-strategist": len([l for l in ALL_PREMIUM_LESSONS.values() if l.get("course_id") == "course-strategist"])
        }
    }
